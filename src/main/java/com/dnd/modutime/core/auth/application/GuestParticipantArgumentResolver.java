...
/**
 * {@link GuestParticipant} 어노테이션이 붙은 파라미터에 Guest JWT 토큰의 {@link GuestInfo}를 주입합니다.
 * 토큰의 roomUuid와 요청 경로의 roomUuid가 일치하는지 검증합니다.
 *
 * @deprecated {@link RoomParticipantArgumentResolver}로 대체되었습니다. Guest/OAuth 통합 지원.
 */
@Deprecated
@Component
...
public class GuestParticipantArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter,
        ...
        var guestInfo = extractGuestInfo(token);
        var roomUuidPathVariable = parameter.getParameterAnnotation(GuestParticipant.class).roomPathVariable();
        validateRoomUuid(request, guestInfo, roomUuidPathVariable);
        return guestInfo;
    }
    ...
    private GuestInfo extractGuestInfo(String token) {
        ...
        return new GuestInfo(parts[1], parts[2]);
    }

    @SuppressWarnings("unchecked")
    private void validateRoomUuid(HttpServletRequest request, GuestInfo guestInfo, String roomUuidPathVariable) {
        ...
        if (pathRoomUuid != null && !pathRoomUuid.equals(guestInfo.roomUuid())) {
