...
public class CandidateDateTimeConvertor {
    //
    public List<CandidateDateTimeDto> convert(List<DateInfo> dateInfos) {
        ...
        for (int i = 1; i < dateTimeInfosDto.size(); i++) {
            ...
            if (preParticipantNames.size() > 0) {
                candidateDateTimesDto.add(new CandidateDateTimeDto(
                        preDateTime.toLocalDate(), preStartTime, preEndTime, preParticipantNames)
                );
            }
