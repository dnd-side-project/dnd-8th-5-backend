...
3. **`gradle.properties` 복사** — modutime 프로젝트라면 메인 저장소의 `gradle.properties`를 워크트리 루트로 항상 복사한다.
   ```bash
   cp /Users/dio.kim/Documents/private/repository/modutime/gradle.properties <worktree-path>/gradle.properties
   ```

4. **요약 보고** — 사용자에게 다음 두 가지를 알려준다.
   - 워크트리 절대 경로
   - 새로 만들어진 브랜치 이름

5. **다음 작업 묻기** — "이 워크트리에서 무슨 작업부터 시작할까요?" 같은 짧은 질문으로 사용자의 의도를 확인하고 거기서부터 이어간다.
